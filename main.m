...
[path_real pathLength frequency] = GenerateRealPath('database', 'route.mat');
...
hold on; 
scatter(path_obser(index_in, 1), path_obser(index_in, 2), 0.7, 'g', 'filled'); 
scatter(path_obser(index_out, 1), path_obser(index_out, 2), 0.7, 'b', 'filled'); 
hold off;
...
% distance = norm(path_obser(1, :) - path_obser(2, :)); theta = pi / length(path_obser);
...
for cnt = 2: length(path_obser) 
    ...
    [prtcle_cons weight_cons] = UpdateParticle(prtcle_cons, weight_cons, path_cons(cnt-1, :), ...
        path_real(cnt, :), path_obser(cnt,:) - path_obser(cnt-1, :), boundPos, 0);
...
for cnt = 2: length(path_obser)
    ...
    % Canculate the weight of particles and resample the particles
    [prtcle_sign weight_sign] = UpdateParticle(prtcle_sign, weight_sign, path_sign(cnt-1, :), ...
        path_real(cnt, :), path_obser(cnt, :) - path_obser(cnt-1, :), boundPos, 1, signType, signPos);
    % Canculate the path according to the particle set
